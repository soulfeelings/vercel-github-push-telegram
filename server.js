...
const requestListener = function (req, res) {
    if (req.url === '/') {
        ...
        const usersArray = getUsersFromOM();
...
function addUserToFile(userString) {
    const usersFromFile = fs.readFileSync('./users.txt', 'utf-8');
    if (!usersFromFile) {
...
function addUserToOM(userString) {
    if (!usersDB) {
        usersDB = userString;
    } else {
        usersDB = usersDB + "\n" + userString;
...
function checkUser(phone) {
    const usersFromFile = getUsersFromOM();
    return usersFromFile.find((user) => {
        return user.phone === phone;
    });
}
function getMsgsFromFile() {
    return fs.readFileSync('./messages.txt', 'utf-8');
}
...
function saveMessageToFile(msg) {
    const name = './messages.txt';
    const messages = fs.readFileSync(name, 'utf-8');
    if (!messages) {
